...
namespace MediaInfoKeeper.ScheduledTask
{
    public class BangumiCharacterRefreshTask : IScheduledTask
    {
        ...
        public async Task Execute(CancellationToken cancellationToken, IProgress<double> progress)
        {
            ...
            var allItems = Plugin.LibraryService.FetchScheduledTaskLibraryItems(taskScope)
                .Where(i => i is Series || i is Movie)
                .Where(i => !string.IsNullOrWhiteSpace(i.GetProviderId(MetadataProviders.Tmdb)))
                .GroupBy(i => i.GetProviderId(MetadataProviders.Tmdb))
                .Select(g => g.First())
                .ToList();

            var total = allItems.Count;
            logger.Info("Bangumi 角色增强计划任务：共找到 {0} 个条目", total);
