...
6. **三方合并 base 必须用上游真实 v1.7.4.9** — fork 可能重写了本地 `v1.7.4.9` tag（指向含 Bangumi 的同步提交，而非上游干净版本），三方合并时 base 应使用上游的真实 v1.7.4.9 commit hash（当前为 `3d88d79`）。用 `git rev-parse upstream/v1.7.4.9` 确认，不要用本地 tag `v1.7.4.9`
...
```bash
# 确认上游真实 v1.7.4.9 commit（不要用本地 tag，fork 可能重写了）
UPSTREAM_BASE=$(git rev-parse upstream/v1.7.4.9)
echo "UPSTREAM_BASE=$UPSTREAM_BASE"

# 准备 base / theirs 目录
mkdir -p /tmp/mik-merge/base /tmp/mik-merge/theirs
...
for f in "${FILES[@]}"; do
  d=$(dirname "$f")
  mkdir -p /tmp/mik-merge/base/$d /tmp/mik-merge/theirs/$d
  git show $UPSTREAM_BASE:$f > /tmp/mik-merge/base/$f 2>/dev/null
  git show HEAD:$f > /tmp/mik-merge/theirs/$f 2>/dev/null
done

# 从备份恢复 fork 版文件到工作区
tar -xzf /tmp/mik-current.tgz -C /workspace/MediaInfoKeeper "./${FILES[0]}" "./${FILES[1]}" "./${FILES[2]}" "./${FILES[3]}" "./${FILES[4]}" "./${FILES[5]}" "./${FILES[6]}" "./${FILES[7]}" "./${FILES[8]}" "./${FILES[9]}" "./${FILES[10]}"

# 三方合并（冲突标记写入工作区文件，无冲突则直接覆盖）
for f in "${FILES[@]}"; do
  git merge-file "$f" "/tmp/mik-merge/base/$f" "/tmp/mik-merge/theirs/$f"
done
