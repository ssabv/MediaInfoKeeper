...
namespace MediaInfoKeeper.Options
{
    public class MetaDataOptions : EditableOptionsBase
    {
        ...
        [DisplayName("启用 Bangumi 角色中文名")]
        [Description("从 Bangumi 获取动漫角色和声优的中文名称，增强 Emby 角色元数据。")]
        public bool EnableBangumiCharacters { get; set; } = false;

        [DisplayName("Bangumi API 地址")]
        [Description("支持镜像。官方: https://api.bgm.tv")]
        [VisibleCondition(nameof(EnableBangumiCharacters), SimpleCondition.IsTrue)]
        public string BangumiApiBaseUrl { get; set; } = "https://bgmapi.anibt.net";

        [DisplayName("覆盖已有中文名")]
        [Description("如果 Emby 已有角色名，是否用 Bangumi 覆盖。")]
        [VisibleCondition(nameof(EnableBangumiCharacters), SimpleCondition.IsTrue)]
        public bool BangumiOverwriteExisting { get; set; } = false;
    }
}
